...
const NavLink: React.FC<{ 
    ...
    // Updated hover colors to match the pink theme
    const colorClass = isDark ? "text-white hover:text-pink-300" : "text-gray-900 hover:text-pink-600 hover:border-pink-600";
...
const Header: React.FC<HeaderProps> = ({ onNavigate, currency, onCurrencyChange, cartCount, onCartClick }) => {
    ...
    return (
        <header className="sticky top-0 z-30 font-sans" onMouseLeave={() => setActiveMegaMenu(null)}>
            ...
            {/* Top Announcement Bar - Updated to #eeb7e4 with White-on-Black icons */}
            <div className="relative z-40 bg-[#eeb7e4] text-black py-1.5 text-[11px] font-semibold tracking-wider border-b border-white/20">
                <div className="container mx-auto px-4 sm:px-6 lg:px-8 flex justify-between items-center">
                    <div className="flex items-center space-x-2">
                        <span className="w-6 h-6 flex items-center justify-center bg-black text-white rounded-full cursor-pointer hover:bg-gray-800 transition-all hover:scale-110" aria-label="Threads"><ThreadsIcon /></span>
                        <span className="w-6 h-6 flex items-center justify-center bg-black text-white rounded-full cursor-pointer hover:bg-gray-800 transition-all hover:scale-110" aria-label="Instagram"><InstagramIcon /></span>
                        <span className="w-6 h-6 flex items-center justify-center bg-black text-white rounded-full cursor-pointer hover:bg-gray-800 transition-all hover:scale-110" aria-label="Facebook"><FacebookIcon /></span>
                        <span className="w-6 h-6 flex items-center justify-center bg-black text-white rounded-full cursor-pointer hover:bg-gray-800 transition-all hover:scale-110" aria-label="WhatsApp"><WhatsAppIcon /></span>
                    </div>
                    <button onClick={() => onNavigate('ofertas')} className="hidden md:block text-center hover:underline truncate px-2 font-medium text-black">
                        <span>
                    ...
                    <div className="flex items-center space-x-4">
                        <select
                            ...
                            className="text-[10px] font-bold bg-transparent border-none focus:ring-0 cursor-pointer text-black uppercase tracking-widest"
                        ...
                        >
                         ...
                         <button onClick={() => onNavigate('contact')} className="hidden sm:block text-[10px] font-bold text-black hover:text-gray-700 transition-colors uppercase tracking-widest">
                            BRAND PARTNER
                        </button>
                    </div>
                </div>
            ...
            <div className="relative z-40 container mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div className="flex justify-between items-center py-4 md:py-6 gap-6">
                    ...
                    <div className="flex-1 flex items-center justify-start">
                        ...
                        <div className="hidden md:block w-full max-w-xs relative group">
                            <input 
                                type="text" 
                                placeholder="Buscar..." 
                                className="w-full bg-gray-50 border border-gray-200 rounded-full py-2 pl-9 pr-4 text-xs focus:border-pink-500 focus:outline-none focus:bg-white transition-all placeholder-gray-400 text-gray-900"
                            />
                            <div className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 group-hover:text-pink-500 transition-colors">
                                <SearchIcon />
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex-1 flex items-center justify-end space-x-2 md:space-x-4">
                        <button className="hidden md:flex flex-col items-center justify-center text-gray-800 hover:text-pink-500 transition-colors group p-2">
                             <UserIcon />
                        </button>
                        ...
                        <button onClick={onCartClick} className="relative flex flex-col items-center justify-center text-gray-800 hover:text-pink-500 transition-colors group p-2" aria-label={`Ver carrito, ${cartCount} artículos`}>
                            <div className="relative">
                                <CartIcon />
                                {cartCount > 0 && (
                                    <span key={cartCount} className={`absolute -top-1.5 -right-1.5 bg-black text-white text-[9px] font-bold w-4 h-4 rounded-full flex items-center justify-center ${cartPulse ? 'animate-bounce' : ''}`}>
                                        {cartCount}
                                    </span>
                                )}
                            </div>
                ...
                <div className="hidden md:block">
                    <nav className="flex justify-center items-center space-x-8">
                        ...
                        <NavLink onClick={() => { onNavigate('ia'); setActiveMegaMenu(null); }} className="text-pink-500 font-bold hover:text-pink-700">Asistente IA</NavLink>
                    </nav>
                </div>
            </div>
